...
export class ArticlePage {
  ...
  constructor(public navParams: NavParams, public cp: CommonProvider,
              ...
              public viewCtrl:ViewController, public datePipe: DatePipe) {
  ...
  init(){
    //设置vip和非vip看的文章的不同版本
    // this.id==3&&this.cp.u.vip>1&&(this.id=12),
    // this.id==12&&this.cp.u.vip<=1&&(this.id=3);
    // this.id==17&&this.cp.u.vip>1&&(this.id=18),
    // this.id==18&&this.cp.u.vip<=1&&(this.id=17);

    this.cp.getData("article/detail", {
      id: this.id, silent: 1, DGAPI:1
    }).then((s:any) => {
      if (s && s.title) {
          this.item = s,
          this.item.content = this.cp.sanitizer.bypassSecurityTrustHtml(this.item.content)/*,
          this.item.extra = this.item.extra ? this.item.extra : {};
          ...
          if(this.cp.getQueryString("btn_click"))
            this.btnClick()

          if(this.item.extra.button_text){
            let __this = this;
            eval(this.item.extra.button_text)
          }
          else
            this.btnText = null;

          if(this.btnText){
            this.item.extra.footer_html = '<button full="" ion-button="" no-margin="" class="button button-ios button-default button-default-ios button-full button-full-ios"><span class="button-inner"> '+this.btnText+' </span><div class="button-effect"></div></button>';
            this.item.extra.footer_height = 44;
          }

          if(this.item.extra.footer_html)
            this.item.extra.footer_html = this.cp.sanitizer.bypassSecurityTrustHtml(this.item.extra.footer_html);

          if(this.item.extra.footer_height){
            let scr = ele.querySelector(".scroll-content");
            scr.setAttribute('style', scr.getAttribute('style') + '; margin-bottom:' + this.item.extra.footer_height + 'px');
          }

          this.video = this.item.extra.video;
          if(this.video) this.video=this.cp.sanitizer.bypassSecurityTrustResourceUrl(this.video);
          */
          this.cp.isWechat() /*&& this.item.extra.can_share*/ && this.api.register({
  ...
  // suc(){
  //   this.cp.temp.center_time = 0;
  //   this.cp.toast(this.cp.u.vip_expire_text||this.cp.u.seller_vip_expire_text?"续费成功":"成功加入会员"),this.cp.pop()
  // }

  // btnClick() {
  //   if(this.item.extra.button_click){
  //     let __this=this;
  //     eval(this.item.extra.button_click);
  //   }
  // }
}
