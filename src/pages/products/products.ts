...
export class ProductsPage {
  ...
  toggleCategories(){
    ...
    if(typeof this.categories=='undefined')
      this.cp.getData("category/getlist/",{DGAPI: 1}).then(t=>{this.categories=t})
  }
  setCategories(k){
  ...
  doInfinite(){
    ...
    this.cp.getData("product/"+(this.params.keyword?"searchlist/":"getlist/"),this.params).then(e=>{
      this.res=e, this.items = this.params.p == 1 ? (this.viewCtrl.contentRef().nativeElement.querySelector('.scroll-content').setAttribute('style','margin-top:calc('+ (this.cp.plt.is("cordova") && this.cp.plt.is("ios")?20:0)+'px + ' + (this.res.data.length?83:44)+'px); margin-top:calc(constant(safe-area-inset-top) + ' + (this.res.data.length?83:44)+'px); margin-top:calc(env(safe-area-inset-top) + ' + (this.res.data.length?83:44)+'px)'), this.res.data, this.res.data) : this.items.concat(this.res.data);
