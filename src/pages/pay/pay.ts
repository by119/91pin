...
export class PayPage {
  params:any = {};
  subjects:any = [];
  subject:any = {icon:'./assets/imgs/logo160.png', price:0};
  ...
  ionViewWillEnter(){
    this.cp.checkLogin().then(loaded=>{
      ...
      if(this.cp.isWechat())
        ...
        this.params.type = this.params.type || 2;
      ...
      // }else 
      if(this.params.id){
        this.cp.getData('subject/getlist',{id: this.params.id, DGAPI:1}).then((res:any)=>{
          if(!res.data.length || !res.data[0].price) return this.cp.pop();
          
          //会自动调用 subjectChange
          this.params.id = res.data[0].id;
  ...
  ionViewDidLeave(){
    if(!this.suc && this.last_code_id){
      this.cp.getData("promo_code/unlock", {silent:1, id: this.last_code_id,DGAPI:1}).then(res=>{})
    }
  }
  subjectChange(){
  ...
  promoCode(){
    ...
    this.cp.getData("promo_code/check", Object.assign({DGAPI:1},this.params)).then((n:any)=>{
      //切换优惠码，将上一个优惠码unlock
      if(this.last_code_id)
        this.cp.getData("promo_code/unlock", {silent:1, id: this.last_code_id,DGAPI:1}).then(res=>{}), this.last_code_id = 0
